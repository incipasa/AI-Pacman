        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                for c in self.corners:
                    if c == new_pos and c not in new_corners:
...
def euclideanHeuristic_(position, problem, i, info={}):
    "The Euclidean distance heuristic for a PositionSearchProblem"
    xy1 = position #tuple 
    xy2 = problem.corners[i] #tuple
    #xy2_x = int (problem.corners[i][0])
    #xy2_y = int (problem.corners[i][1])
    return ((xy1[0][1] - xy2[0]) ** 2 + (xy1[0][1] - xy2[1]) ** 2) ** 0.5
...
def cornersHeuristic(state, problem):
    ...
    if nearest == 1:
        ...
        h_c = h_c + manhattanDistance(corners[0],corners[3])
        h_c = h_c + manhattanDistance(corners[3],corners[2])
...
def foodHeuristic(state, problem):
    ...
    for i in range(food_length):
        cost  = manhattanDistance(state[0],food_list[i])
        if cost < min_cost:
            min_cost = cost
            nearest = i   

    h_c = min_cost
    nearest_point = food_list[nearest]
    food_list.pop(nearest)
    
    while len(food_list) > 0:
        min_cost = 1000000
        for i in range(actual_food_length):
            cost  = manhattanDistance(nearest_point,food_list[i])
            if cost < min_cost:
                min_cost = cost
                nearest = i   
                nearest_point = food_list[nearest]
    ...
    return h_c #cornersHeuristic(state, problem)
